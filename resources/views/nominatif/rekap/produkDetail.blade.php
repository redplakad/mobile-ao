...
@section('content')
    @php
        ...
        $produk = urldecode(request('produk'));
        $items = $recapData[$produk] ?? collect();
        $totalDeb = $items->sum('total_count');
        $totalBaki = $items->sum('total_sum');
        $totalNPL = $items->sum('npl_sum');

        $url = route('nominatif.rekap.produk', ['branch_code' => $branch_code, 'datadate' => $datadate, 'produk' => request('produk'), 'kolektibilitas' => request('kolektibilitas')]);
    @endphp
    ...
    <div class="w-full mx-auto px-4 py-8">
        ...
        <div class="mb-6">
            <h2 class="font-bold text-md mb-2">{{ $produk }}</h2>
                            ...
                            <tr class="hover:bg-gray-100 transition duration-300 text-xs text-gray-700">
                                <td class="px-2 py-3">
                                    <a href="{{ route('nominatif.cabang', [
                                        ...
                                        'recap' => 'nominatif.rekap.produk.detail'
                                    ]) }}">
...
<script>
    $(document).ready(function () {
        $('#rekap-table').DataTable({
            columnDefs: [
                { targets: [0], searchable: true }, // Instansi
