...
@section('content')
    @php
        ...
        $instansi = urldecode(request('instansi'));
        $items = $recapData[$instansi] ?? collect();
        $totalDeb = $items->sum('total_count');
        $totalBaki = $items->sum('total_sum');
        $totalNPL = $items->sum('npl_sum');

        $url = route('nominatif.rekap.instansi', ['branch_code' => $branch_code, 'datadate' => $datadate, 'instansi' => request('instansi'), 'kolektibilitas' => request('kolektibilitas')]);
    @endphp
    ...
    <div class="w-full mx-auto px-4 py-8">
        ...
        <div class="mb-6">
            <h2 class="font-bold text-md mb-2">{{ $instansi }}</h2>
                            ...
                            <tr class="hover:bg-gray-100 transition duration-300 text-xs text-gray-700">
                                <td class="px-2 py-3">
                                    <a href="{{ route('nominatif.cabang', [
                                        ...
                                        'recap' => 'nominatif.rekap.instansi.detail'
                                    ]) }}">
